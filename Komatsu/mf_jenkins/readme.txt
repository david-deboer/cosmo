*** Jenkins et al.'s Halo Mass Function ***
...
We use the fitting parameters for Delta=180 (Eq.[B3])
...
and simply "./make"
- It will generate executables called "sample," "sample_mf_jenkins" and
"compute_mf_jenkins2"

=========================================================================
A simple program to use "mf_jenkins.f90" (also included as "sample.f90" in
this directory):
...
Another program, "compute_mf_jenkins.f90", for generating dn/dlnRh and dn/dlnMh 
for many R (h^-1 Mpc) and M (h^-1 M_solar) is

PROGRAM Compute_Mf_Jenkins
  USE linearpk
  USE sigma
  ! A sample program for computing dn/dlnRh and dn/dlnMh using 
  ! Jenkins et al.'s formula.
  ! Units: 
  ! - Rh in h^-1 Mpc, 
...
! as a function of R [h^-1 Mpc] and M [omega_matter h^-1 M_solar]...'
  ...
  close(1)
  close(2)
END PROGRAM Compute_Mf_Jenkins

An extended version, "compute_mf_jenkins2.f90", computes dn/dlnRh and dn/dlnMh 
at some redshift. The input power spectrum is given at z=30. 

PROGRAM Compute_Mf_Jenkins2
  USE cosmo
  USE linearpk
  USE sigma
  USE growth
  ! A sample program for computing dn/dlnRh and dn/dlnMh using 
  ! Jenkins et al.'s formula at some redshift.
  ! Units: 
  ! - Rh in h^-1 Mpc, 
  ! - Mh in h^-1 M_solar, 
  ! - dndlnRh in h^3 Mpc^-3, and
...
! as a function of R [h^-1 Mpc] and M [h^-1 M_solar] respectively...'
  ...
  close(1)
  close(2)
END PROGRAM Compute_Mf_Jenkins2

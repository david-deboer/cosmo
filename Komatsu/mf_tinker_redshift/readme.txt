...
December 16, 2010: E.Komatsu

Ref. Tinker et al., ApJ, 688, 709 (2008)
     Note that "mf(lnnu)" here corresponds to 0.5*f(sigma) in this references.

We use the fitting parameters for Delta=200 with redshift evolution 
corrections given by Eq.(5)-(8) of Tinker et al. (2008).
...
- mf(lnnu,z) = (halo multiplicity function)
...
function, mf(lnnu,z), as a double-precision function. The argument, lnnu,
...
(9) Compute dn/dlnRh as (3/4pi)*dln(nu)/dlnRh*mf(lnnu,z)/Rh^3
...
! as a function of R [h^-1 Mpc] and M [omega_matter h^-1 M_solar]...'
  ...
  do lnRh=lnR1,lnR2,0.01
     ...
     dlnnudlnRh = -dble(dlnsigma2dlnRh)     ! dln(nu)/dlnRh
     Rh=exp(lnRh) ! h^-1 Mpc
     dndlnRh = (3d0/4d0/3.1415926535d0)*dlnnudlnRh*mf(lnnu,0d0)/dble(Rh)**3d0
...
! as a function of R [h^-1 Mpc] and M [h^-1 M_solar] respectively...'
  ...
  do lnRh=lnR1,lnR2,0.01
     ...
     dlnnudlnRh = -dble(dlnsigma2dlnRh)     ! dln(nu)/dlnRh
     Rh=exp(lnRh) ! h^-1 Mpc
     dndlnRh = (3d0/4d0/3.1415926535d0)*dlnnudlnRh*mf(lnnu,zout)/dble(Rh)**3d0
     write(1,'(2E13.5)') Rh,dndlnRh
     dndlnMh = dndlnRh/3d0 ! in units of h^3 Mpc^-3
     Mh = (4d0*3.1415926535d0/3d0)*2.775d11*Rh**3d0 ! in units of omega_matter h^-1 M_solar

...
/* From http://background.uchicago.edu/~whu/transfer/power.c */
/* via http://risa.stanford.edu/readings_463.php */
...
float calc_delta_H(float n);

#include <math.h>
#include <stdio.h>
#include <stdlib.h>
enum{FN, Z_IN, H_IN, N_IN, K_IN};

/* simple wrapper to provide T(k) given k, z and h.  Use cosmology consistent with nedclass */
int main(int argc, char *argv[])
{
	int TFout;
	float k, z, h, n;
	float omega_hdm, omega_matter, omega_baryon, degen_hdm, omega_lambda;
	float delta_H, TF, tmp;
	...
	omega_matter = 0.27;
	omega_baryon = 0.044;
	omega_hdm = 4.165E-5/h/h;
	degen_hdm = 3.0;
	omega_lambda = 1.0 - omega_matter -  omega_hdm;
	TFmdm_set_cosm(omega_matter, omega_baryon, omega_hdm, degen_hdm, omega_lambda, h, z);
	...
	{
		printf("This will eventually allow many k's to be computed...\n");
	}
	
	delta_H = calc_delta_H(n);
	...
	tmp = TF*256.0;
	TFout = (int) tmp;
	return TFout;  //This is a bad idea...
}
...
float   alpha_gamma,	/* sqrt(alpha_nu) */
	...
	omega_totalm,/* Omega matter total = Omega_matter + omega_hdm */
	...
	sets many global variables for use in TFmdm_onek_mpc() */
{
    ...
    if (omega_baryon<0.0) {
		fprintf(stderr,"TFmdm_set_cosm(): Negative omega_baryon set to trace amount.\n");
		qwarn = 1;
    }
    if (omega_hdm<0.0) {
		fprintf(stderr,"TFmdm_set_cosm(): Negative omega_hdm set to trace amount.\n");
		qwarn = 1;
    }
    if (hubble<=0.0) {
	...
	if (redshift<=-1.0) {
		...
		fprintf(stderr,"TFmdm_set_cosm(): Large redshift entered.  TF may be inaccurate.\n");
		qwarn = 1;
    }
	...
	omega_totalm = omega_matter + omega_hdm;
...
float calc_delta_H(float n)
/*This calculates delta_H from various equations in Eisenstein and Hu*/
/*   specifically Eq 30 */
{
	float omegaTerm, nTerm,omega_H, nTilde;
	
	nTilde = n - 1.0;
	omegaTerm = pow(omega_totalm,-0.785-0.05*log(omega_totalm));
	nTerm = exp(-0.95*nTilde-0.169*nTilde*nTilde);
	
	omega_H = 1.94E-5*omegaTerm*nTerm;
